...
export class CodeEditorPreview extends HTMLElement {
    constructor() {
        ...
        if (!document.querySelector('link[href*="CodeEditorPreview.css"]')) {
            ...
            link.rel = 'stylesheet';
            // 修复iframe沙箱环境中的URL构造问题
            try {
                link.href = new URL('./CodeEditorPreview.css', import.meta.url).href;
            } catch (error) {
                // 在沙箱环境中，使用相对路径作为备用方案
                console.log('沙箱环境中的URL构造问题: 正在尝试使用相对路径');
                link.href = '/src/components/ComponentsPage/CodeEditorPreview/CodeEditorPreview.css';
            }
    ...
    static get observedAttributes() {
        return [
            ...
            'show-toolbar',
            'debounce-delay', 'default-code', 'default-code-path',
            'instructions'
        ];
    }
    ...
    parseAttributes() {
        ...
        // 处理默认代码：优先使用 default-code-path，其次 default-code，最后默认示例
        const defaultCodePath = this.getAttribute('default-code-path');
        const defaultCodeContent = this.getAttribute('default-code');
        const defaultCodeValue = defaultCodePath || defaultCodeContent || this.getDefaultCode();
        ...
        return {
            ...
            debounceDelay: parseInt(this.getAttribute('debounce-delay')) || 300,
            defaultCode: defaultCodeValue,
            defaultCodePath: defaultCodePath,
            originalDefaultContent: defaultCodeContent,
    ...
    handleClick(event) {
        ...
        this.controller.handleAction(action).catch(error => {
    ...
    setDefaultCode(code, language) { 
        return this.controller?.setCode(code, language) || false;
    }
