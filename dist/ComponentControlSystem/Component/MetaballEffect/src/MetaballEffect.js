...
class MetaballContainer extends HTMLElement {
    constructor() {
        ...
        this.blurIntensity = 6;  // 降低默认模糊强度
        this.contrastLevel = 25; // 调整默认对比度
        this.init();
    }
        ...
        this.shadowRoot.innerHTML = `
            <style>
                :host {
                    ...
                    position: relative;
                    /* 核心：高对比度滤镜实现融球效果 */
                    filter: contrast(${this.contrastLevel}) blur(0.3px);
                    background: inherit;
                }
     ...
     */
    setBlur(intensity) {
        this.blurIntensity = Math.max(2, Math.min(15, intensity));
        this.updateStyles();
    }
     ...
     */
    setContrast(level) {
        this.contrastLevel = Math.max(10, Math.min(40, level));
        this.updateStyles();
    }
